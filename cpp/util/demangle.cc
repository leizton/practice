...
std::string demangleFromFile(const int fd, uint64_t offset) {
  ...
  char buf[BUFSIZE], demangled_buf[BUFSIZE];
  if (!readFromOffset(fd, buf, BUFSIZE-1, offset) > 0) {
    return "";
  }
  buf[BUFSIZE-1] = 0;
  demangle(buf, demangled_buf, BUFSIZE-1);
  demangled_buf[BUFSIZE-1] = 0;
  return std::string(demangled_buf);
}