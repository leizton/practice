...
好的设计：实现后被修改得少，被扩展得多
一个模糊的定性目标，变成清晰的定量目标，达到定量目标的多个思想`拆解`成多个方案
业务初期可以单机搞大量业务逻辑，业务增长期就必须做架构拆分了
建立关注和维持奔向目标的系统，而不是关注目标，尤其应避免制定考试性的量化目标。奔向目标的系统是过程，目标只是某个时间点的结果
很多事情坚持做久了都会有意想不到的收获
股票交易终归是一门组合理性价值分析和心理调控的艺术
智慧来源于解决问题，即使是哲学和宗教也是如此
软件中面对本质复杂用拆解成子问题手段；面对偶然复杂用分层、解耦
性能问题、用户消费指标下降问题、用户流失问题，这都是非具体的泛问题，不能通过某个方案方法就一劳永逸地解决。
  就像人的健康问题，不能说去医院打一针后一辈子就平安无事。不同时期有不同病因，就需要不同的应对方法。
  非具体问题先要找到具体原因才能给出具体的解决方法
leader的能力：⒈协调各方能力使全体奔向同一个目标没有内耗且协作最优化如韩信 ⒉进度把控能力 ⒊人尽其用
看一个人能力大小，首先看他如何认知事物的格局大小，是否看得广(系统性、多角度、对立方角度、大局观)，是否看得深远(未来发展、趋势、新事物)
如何让人做事：鼓励他们的梦想，摆脱他们的过错，减轻他们的恐惧，肯定他们的推测，攻击他们的敌人
区别就在于是当产品来做还是当工具来做，产品是好用才行，工具是能用就行

学习技巧
  - 故事学习法，用逻辑推理和结构费曼学习法的本质
  - “如果要了解其他系统，可以从三个层次去学习，先大概了解架构，然后深入到一些具体的细节问题，最后如果有时间还可以继续深入到代码级别”
  - 阅读学习注意内容要有深度信息量，不是简单罗列现象、解决方法。对于现象要有本质和系统性的思考。对于解决方法(算法)要有具体例子中的问题分析和怎样解决的
  - 机器学习技巧: 解决一个真实案例、有一个好的数据集

工程技巧
  - 费米技巧：解决复杂问题之道，问题拆解成可执行的步骤. 从大的层面(系统层面)入手，确定大方向后逐步细化
  - 应用优先
  - 不要上云，因为云成本太高，云附加太多中小型业务无需的三高和分布式服务功能
  - 不要过度拆分服务，因为维护成本太高
  - 容器化部署一定要做好资源隔离，资源包括cpu mem network disk_io
  - 重构和流程优化时，尽可能小改动，抑制整理代码的想法；测试准备充分再上线
  - 当我们能提出建设性的改进意见或创造性的方法时，可以认为掌握了这块
  - 用bitmap的方法观察一个列表(序列)在处理流程中的修改情况，一次观察一个维度，每个bit位表示列表中该原始的维度值
  - 数据分析就是把指标不断细化下拆的过程
  - 召回/过滤候选/排序(粗排+精排)两阶段设计模式
  - 工厂设计模式 QosMetricFactory
  - 试探法. 比如求解二元一次方程Ax=By+C的整数近似解
  - 二八定律(帕累托原则)的应用. 时序监控里tag组合虽然维度不高但空间数据量巨大，从频率概率上对少数组合做特殊优化
  - 需要处理OnTimeout时，用一个线程通过时间轮管理processor，超时调用processor->OnTimeout()

软件设计技巧
  - 面向接口(功能)编程
  - 面向数据编程：先抽象核心数据结构(如user_behavior_history)，然后提供数据访问(view_duration)、处理(stat)接口

软件开发技巧
  - 动态配置化
  - 框架通过wrap函数调用业务接口，调用链：frame -> doProcess -> process
  - 用类多态代替函数指针，方便以后通过类成员变量传递数据，以及通过增加类方法进行扩展
  - 倒数计数器
  - 状态机. 理解状态流转(数据multi-pipeline流程)就是理解业务
  - 在一次请求处理中传递和管理context数据
  - 避免重复或相似代码
  - 充分利用TLD中的trace信息
  - 任何使用线程池提交并发任务的地方必须命名任务(c++需要加上类似日志的文件名/行号)，用于在出现任务队列堵住、任务数持续上涨、任务处理失败等问题时快速定位
...
共存是目的，清零是手段。目标长期不变，手段灵活修改
开窍的感觉就是你可以充满自信很精炼地把内核告诉别人，并且别人很容易就懂了。(不能再用如人饮水了)
费米思想
